export default {
  'ko': {
    meta: {
      title: '마우스 클릭 테스트 - CPS 측정기 | 클릭속도 측정 · 클릭 테스트 · 우클릭 테스트',
      description: '무료 온라인 마우스 클릭 테스트(CPS 테스트). 클릭속도 측정, 좌클릭·우클릭 속도 실시간 측정, CPS 측정기. 광클 테스트, 클릭 스피드 테스트, 마우스 클릭 카운터, 코히 클릭 연습. 게이밍·마인크래프트 PVP 훈련. 다운로드 없이 바로 사용.',
      keywords: '마우스 클릭 테스트, 클릭속도 측정, 클릭 속도 측정, 클릭 테스트, cps 테스트, cps test, cps 측정기, cps trainer, 마우스 클릭 카운터, 클릭 스피드 테스트, 클릭속도, 클릭 속도, 클릭속도 테스트, 클릭 속도 테스트, 마우스 클릭 속도 테스트, 마우스 속도 테스트, 클릭속도 측정기, 클릭 속도 측정기, 광클 테스트, 우클릭 테스트, 오른쪽 클릭 cps 테스트, 마우스 더블클릭 테스트, 마우스 더블 클릭 테스트, 클릭 사이트, 마우스 테스트 사이트, 마우스테스트 사이트, click test, click speed test, mouse click test, mouse click counter, click counter, right click cps test, left click cps test, online cps test, free cps test, kohi click test, 마인크래프트 CPS'
    },
    title: '마우스 클릭 속도 테스트',
    subtitle: '실시간 CPS 측정으로 정확한 마우스 클릭 속도를 확인하세요',
    leftClick: '좌클릭',
    rightClick: '우클릭',
    leftClickSpeed: '좌클릭 속도',
    rightClickSpeed: '우클릭 속도',
    clicksPerSecond: '{count} 클릭/초',
    totalClicks: '총 클릭',
    clickHere: '여기를 클릭하여 테스트 시작',
    testing: '측정 중...',
    testHint: '좌클릭 또는 우클릭으로 속도 측정',
    reset: '초기화',
    selectLanguage: '언어 선택',
    
    howToUse: {
      title: '사용 방법',
      step1Title: '테스트 영역 클릭',
      step1Desc: '위의 테스트 영역을 마우스 좌클릭 또는 우클릭하세요',
      step2Title: '속도 확인',
      step2Desc: '실시간으로 초당 클릭 수(CPS)가 표시됩니다',
      step3Title: '기록 비교',
      step3Desc: '최고 기록을 달성하고 친구들과 비교해보세요'
    },
    
    features: {
      title: '주요 기능',
      accurate: '정확한 실시간 측정',
      gaming: '게이밍 최적화',
      mobile: '모바일 지원',
      free: '완전 무료',
      multilang: '다국어 지원',
      privacy: '개인정보 보호'
    },
    
    faq: {
      title: '자주 묻는 질문',
      q1: 'CPS란 무엇인가요?',
      a1: 'CPS는 Clicks Per Second(초당 클릭 수)의 약자로, 1초 동안 마우스를 몇 번 클릭할 수 있는지를 측정하는 지표입니다. 게임에서 중요한 요소이며, 일반적으로 5-10 CPS가 평균적인 수준입니다.',
      q2: '좋은 CPS 점수는 얼마인가요?',
      a2: '일반 사용자는 보통 5-7 CPS, 게이머는 8-12 CPS, 프로 게이머는 15 CPS 이상을 기록합니다. 지터 클릭이나 버터플라이 클릭 같은 고급 기술을 사용하면 20 CPS 이상도 가능합니다.',
      q3: '클릭 속도를 향상시키는 방법은?',
      a3: '규칙적인 연습이 가장 중요합니다. 지터 클릭(손목을 떨며 클릭), 버터플라이 클릭(두 손가락 교대 클릭), 드래그 클릭(마우스 버튼을 드래그) 등의 기술을 연습하세요. 또한 적절한 게이밍 마우스 사용도 도움이 됩니다.',
      q4: '왜 우클릭도 테스트하나요?',
      a4: '일부 게임(예: 마인크래프트)에서는 우클릭 속도도 중요합니다. 블록 배치나 특정 동작에 우클릭이 사용되기 때문에 양쪽 버튼 모두 테스트할 수 있도록 제공합니다.',
      q5: '마인크래프트에서 CPS가 왜 중요한가요?',
      a5: '마인크래프트 PVP에서 높은 CPS는 더 많은 콤보와 넉백을 만들어 전투에서 유리합니다. 대부분의 경쟁 서버에서 8-10 CPS 이상이 권장되며, 15 CPS 이상이면 매우 우수한 수준입니다.'
    },
    
    footer: {
      about: 'About Us',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service'
    }
  },
  
  'en': {
    meta: {
      title: 'CPS Test - Click Speed Test | Mouse Click Counter & Right Click Test',
      description: 'Free online CPS test and click speed test. Measure left click and right click CPS in real-time. Mouse click counter, click test, clicking speed test, clicker test. Kohi click practice, Minecraft PVP, gaming. No download required.',
      keywords: 'cps test, click test, click speed test, clicking speed test, mouse click test, mouse click speed test, mouse click counter, click counter, right click cps test, left click cps test, left click speed test, online cps test, free cps test, cps test online, click speed test online, mouse click test online, kohi click test, cps trainer, clicks per second, mouse clicker test, double click test, Minecraft CPS'
    },
    title: 'Mouse Click Speed Test',
    subtitle: 'Measure your exact click speed with real-time CPS tracking',
    leftClick: 'Left Click',
    rightClick: 'Right Click',
    leftClickSpeed: 'Left Click Speed',
    rightClickSpeed: 'Right Click Speed',
    clicksPerSecond: '{count} clicks/sec',
    totalClicks: 'Total Clicks',
    clickHere: 'Click here to start testing',
    testing: 'Testing...',
    testHint: 'Left or right click to measure speed',
    reset: 'Reset',
    selectLanguage: 'Select Language',
    
    howToUse: {
      title: 'How to Use',
      step1Title: 'Click Test Area',
      step1Desc: 'Left click or right click the test area above',
      step2Title: 'Check Speed',
      step2Desc: 'Your CPS (clicks per second) will display in real-time',
      step3Title: 'Compare Score',
      step3Desc: 'Beat your high score and compare with friends'
    },
    
    features: {
      title: 'Key Features',
      accurate: 'Accurate Real-time Measurement',
      gaming: 'Gaming Optimized',
      mobile: 'Mobile Friendly',
      free: 'Completely Free',
      multilang: 'Multi-language Support',
      privacy: 'Privacy Protected'
    },
    
    faq: {
      title: 'Frequently Asked Questions',
      q1: 'What is CPS?',
      a1: 'CPS stands for Clicks Per Second, measuring how many times you can click your mouse in one second. It\'s an important metric in gaming, with 5-10 CPS being average.',
      q2: 'What is a good CPS score?',
      a2: 'Regular users typically achieve 5-7 CPS, gamers reach 8-12 CPS, and professional players can exceed 15 CPS. With advanced techniques like jitter clicking or butterfly clicking, 20+ CPS is possible.',
      q3: 'How can I improve my clicking speed?',
      a3: 'Regular practice is key. Learn techniques like jitter clicking (vibrating your wrist), butterfly clicking (alternating two fingers), or drag clicking (dragging across the mouse button). A good gaming mouse also helps.',
      q4: 'Why test right click too?',
      a4: 'In some games like Minecraft, right click speed matters for block placement and certain actions. We provide testing for both buttons to cover all gaming scenarios.',
      q5: 'Why is CPS important in Minecraft?',
      a5: 'In Minecraft PVP, higher CPS creates more combos and knockback, giving you combat advantage. Most competitive servers benefit from 8-10+ CPS, with 15+ CPS being excellent.'
    },
    
    footer: {
      about: 'About Us',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service'
    }
  },
  
  'zh-CN': {
    meta: {
      title: 'CPS测试 - 鼠标点击速度测试 | 点击计数器与右键点击测试',
      description: '免费在线CPS测试与点击速度测试。实时测量左键、右键每秒点击次数，点击计数器。支持Minecraft PVP、电竞手速训练。无需下载，即开即用。',
      keywords: 'CPS测试, 点击速度测试, 点击测试, 鼠标点击测试, 鼠标点击速度测试, 测试点击速度, 点击速度, 点击计数器, 右键点击测试, 左键测试, 在线CPS测试, 免费CPS测试, 每秒点击次数, 手速测试, kohi点击测试, 游戏鼠标测试, Minecraft CPS'
    },
    title: '鼠标点击速度测试',
    subtitle: '通过实时CPS测量了解您的精确点击速度',
    leftClick: '左键点击',
    rightClick: '右键点击',
    leftClickSpeed: '左键点击速度',
    rightClickSpeed: '右键点击速度',
    clicksPerSecond: '{count} 次/秒',
    totalClicks: '总点击数',
    clickHere: '点击这里开始测试',
    testing: '测试中...',
    testHint: '左键或右键点击以测量速度',
    reset: '重置',
    selectLanguage: '选择语言',
    
    howToUse: {
      title: '使用方法',
      step1Title: '点击测试区域',
      step1Desc: '使用鼠标左键或右键点击上方测试区域',
      step2Title: '查看速度',
      step2Desc: '您的CPS（每秒点击次数）将实时显示',
      step3Title: '比较成绩',
      step3Desc: '挑战最高记录并与朋友比较'
    },
    
    features: {
      title: '主要特点',
      accurate: '精准实时测量',
      gaming: '游戏优化',
      mobile: '移动端支持',
      free: '完全免费',
      multilang: '多语言支持',
      privacy: '隐私保护'
    },
    
    faq: {
      title: '常见问题',
      q1: '什么是CPS？',
      a1: 'CPS是Clicks Per Second（每秒点击次数）的缩写，用于测量1秒内可以点击鼠标多少次。这是游戏中的重要指标，5-10 CPS为平均水平。',
      q2: '好的CPS分数是多少？',
      a2: '普通用户通常达到5-7 CPS，游戏玩家可达8-12 CPS，职业选手超过15 CPS。使用抖动点击或蝴蝶点击等高级技巧，可以达到20+ CPS。',
      q3: '如何提高点击速度？',
      a3: '定期练习是关键。学习抖动点击（震动手腕）、蝴蝶点击（两指交替）或拖拽点击（拖动鼠标按键）等技巧。使用好的游戏鼠标也有帮助。',
      q4: '为什么也要测试右键？',
      a4: '在某些游戏（如Minecraft）中，右键速度对于放置方块和某些动作很重要。我们提供双键测试以覆盖所有游戏场景。',
      q5: 'Minecraft中CPS为什么重要？',
      a5: '在Minecraft PVP中，更高的CPS可以产生更多连击和击退效果，在战斗中占据优势。大多数竞技服务器中，8-10+ CPS是推荐水平，15+ CPS则非常优秀。'
    },
    
    footer: {
      about: '关于我们',
      privacy: '隐私政策',
      terms: '服务条款'
    }
  },
  
  'zh-TW': {
    meta: {
      title: '點擊速度測試 - CPS測試 | 滑鼠點擊測試與右鍵點擊測試',
      description: '免費線上CPS測試與點擊速度測試。即時測量左鍵、右鍵每秒點擊次數，點擊計數器。支援Minecraft PVP、電競手速訓練。無需下載。',
      keywords: '點擊速度測試, CPS測試, 點擊測試, 滑鼠點擊測試, 滑鼠點擊速度測試, 測點擊速度, 點擊速度, 點擊計數器, 右鍵點擊測試, 線上CPS測試, 每秒點擊次數, 手速測試, 遊戲滑鼠測試, Minecraft CPS'
    },
    title: '滑鼠點擊速度測試',
    subtitle: '透過即時CPS測量了解您的精確點擊速度',
    leftClick: '左鍵點擊',
    rightClick: '右鍵點擊',
    leftClickSpeed: '左鍵點擊速度',
    rightClickSpeed: '右鍵點擊速度',
    clicksPerSecond: '{count} 次/秒',
    totalClicks: '總點擊數',
    clickHere: '點擊這裡開始測試',
    testing: '測試中...',
    testHint: '左鍵或右鍵點擊以測量速度',
    reset: '重置',
    selectLanguage: '選擇語言',
    
    howToUse: {
      title: '使用方法',
      step1Title: '點擊測試區域',
      step1Desc: '使用滑鼠左鍵或右鍵點擊上方測試區域',
      step2Title: '查看速度',
      step2Desc: '您的CPS（每秒點擊次數）將即時顯示',
      step3Title: '比較成績',
      step3Desc: '挑戰最高記錄並與朋友比較'
    },
    
    features: {
      title: '主要特點',
      accurate: '精準即時測量',
      gaming: '遊戲優化',
      mobile: '移動端支援',
      free: '完全免費',
      multilang: '多語言支援',
      privacy: '隱私保護'
    },
    
    faq: {
      title: '常見問題',
      q1: '什麼是CPS？',
      a1: 'CPS是Clicks Per Second（每秒點擊次數）的縮寫，用於測量1秒內可以點擊滑鼠多少次。這是遊戲中的重要指標，5-10 CPS為平均水平。',
      q2: '好的CPS分數是多少？',
      a2: '普通用戶通常達到5-7 CPS，遊戲玩家可達8-12 CPS，職業選手超過15 CPS。使用抖動點擊或蝴蝶點擊等高級技巧，可以達到20+ CPS。',
      q3: '如何提高點擊速度？',
      a3: '定期練習是關鍵。學習抖動點擊、蝴蝶點擊或拖曳點擊等技巧。使用好的遊戲滑鼠也有幫助。',
      q4: '為什麼也要測試右鍵？',
      a4: '在某些遊戲（如Minecraft）中，右鍵速度對於放置方塊和某些動作很重要。我們提供雙鍵測試以覆蓋所有遊戲場景。',
      q5: 'Minecraft中CPS為什麼重要？',
      a5: '在Minecraft PVP中，更高的CPS可以產生更多連擊和擊退效果，在戰鬥中佔據優勢。大多數競技伺服器中，8-10+ CPS是推薦水平。'
    },
    
    footer: {
      about: 'About Us',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service'
    }
  },
  
  'ja': {
    meta: {
      title: 'CPSテスト - クリック速度テスト | マウスクリックカウンター・右クリックテスト',
      description: '無料オンラインCPSテスト・クリック速度テスト。左クリック・右クリックのCPSをリアルタイム測定。クリックカウンター、クリックテスト。Minecraft PVP・ゲーミング練習。ダウンロード不要。',
      keywords: 'CPSテスト, クリックテスト, クリック速度テスト, マウスクリックテスト, クリックカウンター, 右クリックテスト, 秒間クリック数, オンラインCPSテスト, click test, click speed test, mouse click test, mouse click counter, kohi クリックテスト, Minecraft CPS'
    },
    title: 'マウスクリック速度テスト',
    subtitle: 'リアルタイムCPS測定で正確なクリック速度を確認',
    leftClick: '左クリック',
    rightClick: '右クリック',
    leftClickSpeed: '左クリック速度',
    rightClickSpeed: '右クリック速度',
    clicksPerSecond: '{count} 回/秒',
    totalClicks: '総クリック数',
    clickHere: 'ここをクリックしてテスト開始',
    testing: 'テスト中...',
    testHint: '左または右クリックで速度測定',
    reset: 'リセット',
    selectLanguage: '言語を選択',
    
    howToUse: {
      title: '使用方法',
      step1Title: 'テストエリアをクリック',
      step1Desc: '上のテストエリアを左または右クリックしてください',
      step2Title: '速度を確認',
      step2Desc: 'CPS（秒間クリック数）がリアルタイムで表示されます',
      step3Title: 'スコアを比較',
      step3Desc: '最高記録を目指して友達と比較しましょう'
    },
    
    features: {
      title: '主な機能',
      accurate: '正確なリアルタイム測定',
      gaming: 'ゲーミング最適化',
      mobile: 'モバイル対応',
      free: '完全無料',
      multilang: '多言語対応',
      privacy: 'プライバシー保護'
    },
    
    faq: {
      title: 'よくある質問',
      q1: 'CPSとは何ですか？',
      a1: 'CPSはClicks Per Second（秒間クリック数）の略で、1秒間に何回マウスをクリックできるかを測定する指標です。ゲームでは重要な要素で、5-10 CPSが平均的なレベルです。',
      q2: '良いCPSスコアはいくつですか？',
      a2: '一般ユーザーは通常5-7 CPS、ゲーマーは8-12 CPS、プロプレイヤーは15 CPS以上を記録します。ジッタークリックやバタフライクリックなどの高度な技術を使用すると、20 CPS以上も可能です。',
      q3: 'クリック速度を向上させる方法は？',
      a3: '定期的な練習が重要です。ジッタークリック、バタフライクリック、ドラッグクリックなどの技術を練習しましょう。適切なゲーミングマウスの使用も役立ちます。',
      q4: 'なぜ右クリックもテストするのですか？',
      a4: 'Minecraftなどの一部のゲームでは、ブロック配置や特定のアクションに右クリック速度が重要です。すべてのゲームシナリオをカバーするために両方のボタンをテストします。',
      q5: 'MinecraftでCPSが重要な理由は？',
      a5: 'Minecraft PVPでは、高いCPSがより多くのコンボとノックバックを生み出し、戦闘で有利になります。ほとんどの競技サーバーでは8-10+ CPSが推奨され、15+ CPSは非常に優秀なレベルです。'
    },
    
    footer: {
      about: 'About Us',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service'
    }
  },
  
  'de': {
    meta: {
      title: 'CPS-Test - Klickgeschwindigkeitstest | Mausklick-Zähler & Rechtsklick-Test',
      description: 'Kostenloser Online-CPS-Test und Klickgeschwindigkeitstest. Links- und Rechtsklick-CPS in Echtzeit messen. Klickzähler, Click-Test. Minecraft PVP, Gaming. Kein Download nötig.',
      keywords: 'CPS-Test, Klick-Test, Klickgeschwindigkeitstest, Mausklick-Test, Klickzähler, Rechtsklick-Test, Online CPS-Test, click test, click speed test, mouse click counter, Minecraft CPS'
    },
    title: 'Mausklick-Geschwindigkeitstest',
    subtitle: 'Messen Sie Ihre genaue Klickgeschwindigkeit mit Echtzeit-CPS-Tracking',
    leftClick: 'Linksklick',
    rightClick: 'Rechtsklick',
    leftClickSpeed: 'Linksklick-Geschwindigkeit',
    rightClickSpeed: 'Rechtsklick-Geschwindigkeit',
    clicksPerSecond: '{count} Klicks/Sek',
    totalClicks: 'Gesamtklicks',
    clickHere: 'Hier klicken, um den Test zu starten',
    testing: 'Testen...',
    testHint: 'Links oder rechts klicken, um die Geschwindigkeit zu messen',
    reset: 'Zurücksetzen',
    selectLanguage: 'Sprache wählen',
    
    howToUse: {
      title: 'Anleitung',
      step1Title: 'Testbereich klicken',
      step1Desc: 'Klicken Sie mit der linken oder rechten Maustaste auf den Testbereich oben',
      step2Title: 'Geschwindigkeit prüfen',
      step2Desc: 'Ihre CPS (Klicks pro Sekunde) wird in Echtzeit angezeigt',
      step3Title: 'Punktzahl vergleichen',
      step3Desc: 'Schlagen Sie Ihren Highscore und vergleichen Sie mit Freunden'
    },
    
    features: {
      title: 'Hauptmerkmale',
      accurate: 'Genaue Echtzeitmessung',
      gaming: 'Gaming-Optimiert',
      mobile: 'Mobilfreundlich',
      free: 'Völlig kostenlos',
      multilang: 'Mehrsprachige Unterstützung',
      privacy: 'Datenschutz geschützt'
    },
    
    faq: {
      title: 'Häufig gestellte Fragen',
      q1: 'Was ist CPS?',
      a1: 'CPS steht für Clicks Per Second und misst, wie oft Sie in einer Sekunde mit der Maus klicken können. Es ist eine wichtige Metrik im Gaming, wobei 5-10 CPS durchschnittlich sind.',
      q2: 'Was ist ein guter CPS-Wert?',
      a2: 'Normale Benutzer erreichen typischerweise 5-7 CPS, Gamer 8-12 CPS und professionelle Spieler über 15 CPS. Mit fortgeschrittenen Techniken wie Jitter-Klicken oder Butterfly-Klicken sind über 20 CPS möglich.',
      q3: 'Wie kann ich meine Klickgeschwindigkeit verbessern?',
      a3: 'Regelmäßiges Üben ist der Schlüssel. Lernen Sie Techniken wie Jitter-Klicken, Butterfly-Klicken oder Drag-Klicken. Eine gute Gaming-Maus hilft ebenfalls.',
      q4: 'Warum auch Rechtsklick testen?',
      a4: 'In einigen Spielen wie Minecraft ist die Rechtsklick-Geschwindigkeit für Blockplatzierung und bestimmte Aktionen wichtig. Wir bieten Tests für beide Tasten an.',
      q5: 'Warum ist CPS in Minecraft wichtig?',
      a5: 'In Minecraft PVP erzeugt höheres CPS mehr Combos und Knockback, was Ihnen Kampfvorteile verschafft. Die meisten Wettbewerbsserver profitieren von 8-10+ CPS, wobei 15+ CPS hervorragend ist.'
    },
    
    footer: {
      about: 'About Us',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service'
    }
  },
  
  'fr': {
    meta: {
      title: 'Test CPS - Test de vitesse de clic | Compteur de clics & Test clic droit',
      description: 'Test CPS et test de vitesse de clic en ligne gratuits. Mesurez le CPS du clic gauche et droit en temps réel. Compteur de clics, click test. Minecraft PVP, gaming. Sans téléchargement.',
      keywords: 'test CPS, click test, test de vitesse de clic, test de clic de souris, compteur de clics, test clic droit, test CPS en ligne, clics par seconde, click speed test, mouse click counter, Minecraft CPS'
    },
    title: 'Test de vitesse de clic de souris',
    subtitle: 'Mesurez votre vitesse de clic exacte avec le suivi CPS en temps réel',
    leftClick: 'Clic gauche',
    rightClick: 'Clic droit',
    leftClickSpeed: 'Vitesse de clic gauche',
    rightClickSpeed: 'Vitesse de clic droit',
    clicksPerSecond: '{count} clics/sec',
    totalClicks: 'Total de clics',
    clickHere: 'Cliquez ici pour commencer le test',
    testing: 'Test en cours...',
    testHint: 'Clic gauche ou droit pour mesurer la vitesse',
    reset: 'Réinitialiser',
    selectLanguage: 'Choisir la langue',
    
    howToUse: {
      title: 'Mode d\'emploi',
      step1Title: 'Cliquez sur la zone de test',
      step1Desc: 'Faites un clic gauche ou droit sur la zone de test ci-dessus',
      step2Title: 'Vérifiez la vitesse',
      step2Desc: 'Votre CPS (clics par seconde) s\'affichera en temps réel',
      step3Title: 'Comparez le score',
      step3Desc: 'Battez votre record et comparez avec vos amis'
    },
    
    features: {
      title: 'Caractéristiques principales',
      accurate: 'Mesure précise en temps réel',
      gaming: 'Optimisé pour le gaming',
      mobile: 'Compatible mobile',
      free: 'Entièrement gratuit',
      multilang: 'Support multilingue',
      privacy: 'Confidentialité protégée'
    },
    
    faq: {
      title: 'Questions fréquemment posées',
      q1: 'Qu\'est-ce que le CPS?',
      a1: 'CPS signifie Clicks Per Second, mesurant combien de fois vous pouvez cliquer en une seconde. C\'est une métrique importante dans le gaming, avec 5-10 CPS étant la moyenne.',
      q2: 'Quel est un bon score CPS?',
      a2: 'Les utilisateurs réguliers atteignent généralement 5-7 CPS, les joueurs 8-12 CPS, et les joueurs professionnels dépassent 15 CPS. Avec des techniques avancées, 20+ CPS est possible.',
      q3: 'Comment améliorer ma vitesse de clic?',
      a3: 'La pratique régulière est la clé. Apprenez des techniques comme le jitter click, le butterfly click ou le drag clicking. Une bonne souris gaming aide également.',
      q4: 'Pourquoi tester le clic droit aussi?',
      a4: 'Dans certains jeux comme Minecraft, la vitesse de clic droit est importante pour le placement de blocs et certaines actions. Nous testons les deux boutons.',
      q5: 'Pourquoi le CPS est-il important dans Minecraft?',
      a5: 'Dans Minecraft PVP, un CPS plus élevé crée plus de combos et de knockback, donnant un avantage au combat. La plupart des serveurs compétitifs bénéficient de 8-10+ CPS.'
    },
    
    footer: {
      about: 'About Us',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service'
    }
  }
}
